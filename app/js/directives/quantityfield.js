four51.app.directive('quantityfield', ['$451', 'ProductDisplayService', '$location', function($451, ProductDisplayService, $location){
	var obj = {
		scope: {
			lineitem : '=',
			calculated: '=',
		...
		template: '<div ng-if="lineitem.ID">'+
			...
			scope.validQuantityAddToOrder = function(value, lineItem){
				...
				angular.forEach(scope.$parent.$parent.$parent.user.CustomFields, function(field, index) {
			if(field.Name == "XL1_Codes_Submitted"){
			    scope.codeCount = field.Value;
			}
			});
		
		    if(scope.$parent.$parent.$parent.currentOrder && $location.path() !== "/cart"){
    			angular.forEach(scope.$parent.$parent.$parent.currentOrder.LineItems, function(item, index) {
    				if(item.Product.ExternalID == "XL1-17370"){
    					scope.codeCount = "In cart";
    				}
    			});
		    }
		    
				if(product.ExternalID == "XL1-17370" && scope.codeCount !== "Redeemed" && scope.codeCount !== "In cart" && scope.codeCount < 4)
				{
					scope.lineitem.qtyError = "You must first submit a minimum of 4 reward codes";
					scope.valid = false;
					return scope.valid;
				}
				if(product.ExternalID == "XL1-17370" && scope.codeCount == "Redeemed")
				{
					scope.lineitem.qtyError = "You have already redeemed your free shirt";
					scope.valid = false;
					return scope.valid;
				}
				if(product.ExternalID == "XL1-17370" && scope.codeCount == "In cart")
				{
					scope.lineitem.qtyError = "There is already a shirt in your cart";
					scope.valid = false;
					return scope.valid;
				}
